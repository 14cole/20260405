...
def _build_grid_for_samples(
    ...
    # GRIM tooling expects rcs to be a complex field quantity for coherent math/phase.
    rcs_amp = np.full(shape, np.nan + 1j * np.nan, dtype=np.complex128)
    # Keep linear-power RCS explicitly for power-domain consumers.
    ...
    for row in samples:
        ...
        if not np.isnan(existing_power):
            ...
            existing_amp = rcs_amp[idx]
            if not np.isnan(existing_amp.real) and abs(existing_amp - amp_value) > EPS:
                raise ValueError(
                    f"Duplicate amplitude conflict at az={az}, el=0, f={freq}, pol={polarization}."
                )
            continue
        rcs_power[idx] = lin
        rcs_amp[idx] = amp_value
    ...
    return {
        ...
        "rcs": rcs_amp,
        "rcs_amp": rcs_amp,
        "rcs_real": np.real(rcs_amp).astype(float),
        "rcs_imag": np.imag(rcs_amp).astype(float),
        "rcs_power": rcs_power,
