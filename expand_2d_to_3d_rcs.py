...
Phase support:
- If 2D input contains complex `rcs_amp` (or CSV real/imag columns), `--combine coherent_2d_phase`
...
def _load_2d_rcs_grim(path: str) -> Rcs2DTable:
    ...
    with np.load(path, allow_pickle=False) as data:
        ...
        rcs_raw = np.asarray(data["rcs"]) if "rcs" in data else None
        rcs_power = np.asarray(data["rcs_power"]) if "rcs_power" in data else None

        has_explicit_amp = "rcs_amp" in data or ("rcs_real" in data and "rcs_imag" in data)
        if "rcs_amp" in data:
            rcs_amp = np.asarray(data["rcs_amp"], dtype=np.complex128)
        elif "rcs_real" in data and "rcs_imag" in data:
            rcs_amp = np.asarray(data["rcs_real"], dtype=np.complex128) + 1j * np.asarray(
    ...
    if rcs_amp is not None and rcs_amp.ndim != 4:
        raise ValueError(f"GRIM file '{path}' has unexpected rcs_amp rank {rcs_amp.ndim}; expected 4D")
    ...
    if rcs_amp is None and rcs_raw is not None:
        if rcs_domain == "complex_amplitude":
            rcs_amp = np.asarray(rcs_raw, dtype=np.complex128)
        elif rcs_domain == "":
            # Heuristic for legacy datasets that store complex field directly in `rcs`.
            rcs_c = np.asarray(rcs_raw, dtype=np.complex128)
            imag_max = float(np.nanmax(np.abs(np.imag(rcs_c)))) if rcs_c.size else 0.0
            if imag_max > 1e-10:
                rcs_amp = rcs_c
    ...
    if rcs_power is None:
        if rcs_raw is not None:
            rcs_c = np.asarray(rcs_raw, dtype=np.complex128)
            if rcs_domain == "complex_amplitude":
                rcs_power = np.abs(rcs_c) ** 2
            elif rcs_domain == "linear_rcs":
                rcs_power = np.real(rcs_c)
            elif has_explicit_amp:
                # Older solver files stored linear power in `rcs` and amplitude in `rcs_amp`.
                rcs_power = np.real(rcs_c)
            elif rcs_amp is not None:
                rcs_power = np.abs(rcs_amp) ** 2
            else:
                rcs_power = np.real(rcs_c)
        elif rcs_amp is not None:
            rcs_power = np.abs(rcs_amp) ** 2
    ...
    return Rcs2DTable(
        ...
        by_freq=by_freq,
        amp_by_freq=(amp_by_freq if rcs_amp is not None else None),
    )
...
def _compute_sigma3d_for_direction(
    ...
    if combine_mode == "coherent_2d_phase" and not table.amp_by_freq:
        raise ValueError(
            ...
            "(GRIM with rcs_amp, or CSV with rcs_amp_real/rcs_amp_imag)."
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "coherent_2d_phase uses phase from 2D rcs_amp (if available) plus geometric phase."
        ),
    )
    p.add_argument(
        "--backface",
